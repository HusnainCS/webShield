...
export async function generateScanReport(scan) {
    ...
    return new Promise((resolve, reject) => {
        ...
        doc.fontSize(16).fillColor("#1E2A38")
        ...
        analysis.recommendations.forEach((r, i) => {
            doc.fontSize(14).fillColor("black").text(`${i + 1}. ${r}`);
            doc.moveDown(0.5);
        });
...
// ======================================================
// Offline AI-style analysis — supports all 4 tools
// ======================================================
function generateAnalysis(scan, sslIssues) {
    ...
    if (weak.length > 0) {
        details += "⚠ SSLScan detected outdated or weak TLS protocols/ciphers in use. These weaken encryption and can expose sensitive user data.\n\n";
    ...
    if (openPorts.length > 0) {
        ...
        if (risky.length > 0) {
            details += "⚠ Nmap detected risky open ports (e.g., SSH, FTP, Database ports). These expand the attack surface.\n\n";
            ...
            details += "✓ Only common web ports appear open. Server exposure is minimal.\n\n";
        }
    }
    ...
    if (dirs.length > 0) {
        const sensitive = dirs.filter(d =>
            ["/admin", "/backup", "/config", "/db", "/phpmyadmin", "/logs"].some(s => d.includes(s))
        );

        if (sensitive.length > 0) {
            details += "⚠ Gobuster discovered sensitive directories exposed publicly (admin panels, backups, configs, logs).\n\n";
...
if (skipfish.length > 0) {
    const critical = skipfish.filter(s => {
        ...
        return str.toLowerCase().includes("high") ||
               str.toLowerCase().includes("critical") ||
               str.toLowerCase().includes("vulnerability");
    });

    if (critical.length > 0) {
        details += "⚠ Skipfish reported high or critical vulnerabilities related to the application layer.\n\n";
    ...
    // ======================================================
    if (details === "") {
        details = "✓ No major threats or misconfigurations detected. The website appears to follow standard security practices.\n\n";
    }
